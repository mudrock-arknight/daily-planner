    ...
    "dailySchedule": {
      "周一": {
        ...
        "timeBlocks": [
          ...
          {
            "time": "10:25-12:00",
            "content": "工程力学一（阶段考复习重点）",
            "type": "class",
            "location": "日新楼南 401",
            "detail": "赵升升老师，第3-4节合并"
          },
          {
            "time": "12:00-13:00",
            "content": "午餐 + 休息放松",
...
function verifyWeek13Plan() {
  ...
  console.log('1. 检查第13周数据...');
  console.log('   找到第13周计划');
  console.log();

  console.log('2. 检查每日计划完整性...');
  const dailySchedule = planData.data?.dailySchedule;
  if (!dailySchedule) {
  ...
  if (dailySchedule) {
    Object.entries(dailySchedule).forEach(function(entry) {
      ...
      timeBlocks.forEach(function(block) {
        const isEnglish = 
          block.type === 'study' && (
            block.content.indexOf('英语') !== -1 ||
            block.content.toLowerCase().indexOf('english') !== -1 ||
            (block.detail && block.detail.indexOf('英语') !== -1)
          );
